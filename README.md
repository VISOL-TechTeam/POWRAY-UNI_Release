...
- WiFi Convert: A9는 Duty_Adjust_End 중에도 수신, CONT만 duty 게이트 유지
- WIFI 전용 명령 큐 + ProtocolTask 우선 처리 + WifiTask/Protocol notify
...
- 100% 밝기는 Duty_100per 보정값을 그대로 사용하도록 RS485/DMX/W-DMX/수동 제어 경로 통일
 ...
 - 수동 스위치 모드 진입/해제 시 SMPS 상태 잔류로 LED 출력과 SMPS 상태가 어긋나는 문제 보완
 - SMPS 전류 응답 raw 값을 0.01A 단위와 현장 보정 계수 1.1 기준으로 표시하도록 보정
...
- 업로더 UI를 RS485/W-DMX 그룹으로 분리하여 요청, 종료, 업로드 버튼을 채널별로 구분
- W-DMX 업로드 전송 취소 시 RUN 프레임을 반복 송신하여 업로드 모드 종료 요청을 함께 수행
- W-DMX 업로드 진행 중 RUN 수신 시 부트로더가 실패 처리 및 백업 복구 루틴으로 진입
- RS485 WRITE ACK 유실 대응을 위해 업로더 WRITE 타임아웃 재시도 로직 추가
- RS485 WRITE 재시도 시 부트로더가 이미 기록된 동일 payload를 중복 기록하지 않고 정상 ACK 응답
- RS485 다중 업로드 중 ID별 실패 시 1회 재시도 후 실패 ID를 건너뛰고 다음 ID 업로드 계속 진행
- RS485 업로드 모드 종료 명령도 ID별 1회 재시도 후 실패 ID를 건너뛰도록 개선
- RS485 업로드 세션 타임아웃을 추가하여 통신 중단 시 백업 복구 또는 FAIL 표시 후 리부트
- RS485 업로드 중 RUN 명령 수신 시 현재 APP 유효성만 확인하지 않고 업로드 실패/복구 절차 수행
